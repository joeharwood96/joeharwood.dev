...
export default function LogoMarquee() {
  return (
    <div className="relative w-full overflow-hidden py-8 sm:py-10 md:py-12">
      ...
      <Marquee pauseOnHover className="[--duration:20s] [--gap:3rem] sm:[--gap:4rem] md:[--gap:5rem] lg:[--gap:8rem]">
        {companies.map((company) => (
          <div
            key={company.id}
            className="flex items-center justify-center min-w-[140px] sm:min-w-[160px] md:min-w-[200px] group cursor-default"
          >
            <div className="relative h-10 w-32 sm:h-12 sm:w-40 md:h-16 md:w-48 grayscale group-hover:grayscale-0 transition-all duration-500 opacity-40 group-hover:opacity-100">
              <Image
                src={company.logo}
                alt={company.name}
                fill
